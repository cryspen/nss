...
#define Eurydice_array_to_slice(end, x, t, _ret_t) \
    EURYDICE_SLICE(                                \
        x, 0, end) /* x is already at an array type, no need for cast */
...
#define core_array_equality___Array_A__N___eq(sz, a1, a2, t, _, _ret_t) \
...
#define Eurydice_slice_to_array2(dst, src, _, t_arr, _ret_t) \
    Eurydice_slice_to_array3(                                \
        (result_tryfromslice_flexible*)dst, src, sizeof(t_arr))

static inline void
Eurydice_slice_to_array3(result_tryfromslice_flexible* dst,
                         Eurydice_slice src,
                         size_t sz)
{
    dst->tag = 0;
...
#define core_iter_range__core__ops__range__Range_A__3__next( \
    iter_ptr, t, ret_t)                                      \
...
chunk_next(Eurydice_chunks* chunks, size_t element_size)
{
    ...
    chunks->slice = ((Eurydice_slice){ .ptr = (char*)(chunks->slice.ptr) +
...
#define core_slice_iter__core__slice__iter__Chunks__a__T__70__next(  \
    iter, t, ret_t)                                                  \
