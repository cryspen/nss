 ...
 *
 * - Utility functions were all collected in this header file, and one: verify()
 *   from verify.h was removed as NSS_SecureMemcmp is available.
 * - Only the Kyber-512 and Kyber-768 parameter sets were retained.
 * - The KEM API functions were changed to be more NSS-like and were moved into
 *   another file.
 *
 * Note that KYBER_K must be defined before including this header. */
...
#define KYBER_ETA1 2
#else
#error "KYBER_K must be defined to be a value in {2,3}"
#endif
...
**************************************************/
static void
indcpa_keypair(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
               uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES],
               const uint8_t seed[KYBER_SYMBYTES])
{
    unsigned int i;
