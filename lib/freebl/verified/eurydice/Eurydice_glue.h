...
static inline void
Eurydice_slice_to_array3(result_tryfromslice_flexible *dst, Eurydice_slice src, size_t sz)
{
    dst->tag = 0;
...
core_convert_num__i64_59__from(int32_t x)
{
    return x;
}
static inline int32_t
core_convert_num__i32_56__from(int16_t x)
{
    return x;
}
...
static inline void
core_ops_arith__i32_319__add_assign(int32_t *x0, int32_t *x1)
{
    *x0 = *x0 + *x1;
}
...
#define core_iter_range__core__ops__range__Range_A__3__next(iter_ptr, t) ( \
    ((iter_ptr)->start == (iter_ptr)->end) ? ((core_option_Option__##t){ .tag = core_option_None }) : ((core_option_Option__##t){ .tag = core_option_Some, .f0 = (iter_ptr)->start++ }))
...
chunk_next(Eurydice_chunks *chunks, size_t element_size)
{
...
#define core_slice_iter__core__slice__iter__Chunks__a__T__70__next(iter, t) \
    (((iter)->slice.len == 0) ? ((core_option_Option__Eurydice_slice_##t){ .tag = core_option_None }) : ((core_option_Option__Eurydice_slice_##t){ .tag = core_option_Some, .f0 = chunk_next(iter, sizeof(t)) }))
